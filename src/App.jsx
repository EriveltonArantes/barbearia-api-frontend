...
const apiBaseUrl = "https://barbearia-api-av7p.onrender.com";
...
function PainelBarbeiro({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "telefone", "especialidade", "disponivel", "foto", "bio", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/barbeiros", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/barbeiros/" + editId : apiBaseUrl + "/api/barbeiros";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "telefone", "especialidade", "disponivel", "foto", "bio"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/barbeiros/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              <div className="item-photo-fallback">B</div>
              <img className="item-photo" style={{display: "none"}} src={item.foto} alt="Barbeiro" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🧑‍🔧</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["nome"] ?? "Barbeiro")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Barbeiro</h3>
      ...
      <input type="file" className="upload-input" onChange={e => uploadArquivo(e.target.files[0], v => setForm(f => ({...f, foto: v})), token)} />
      {form["foto"] && <a className="upload-preview-link" href={apiBaseUrl + form["foto"]} target="_blank" rel="noreferrer">Ver arquivo enviado</a>}
...
function PainelCliente({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "telefone", "email", "dataNascimento", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "telefone", "email", "dataNascimento", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
...
function PainelCorte({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [barbeiroList, setBarbeiroList] = React.useState([]);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["barbeiroId", "clienteId", "servico", "dataHora", "duracao", "valor", "status", "avaliacao", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/cortes", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/barbeiros", { headers: authHeaders(token) }).then(r => r.json()).then(setBarbeiroList).catch(() => {});
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/cortes/" + editId : apiBaseUrl + "/api/cortes";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["barbeiroId", "clienteId", "servico", "dataHora", "duracao", "valor", "status", "avaliacao"].forEach(k => { f[k] = item[k] ?? ""; });
    ["barbeiro", "cliente"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/cortes/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["servico"] ?? "Corte")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/cortes/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["servico"] ?? "Corte")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>barbeiroId</b><span>{item["barbeiroId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>avaliacao</b><span>{item["avaliacao"] != null ? Number(item["avaliacao"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>barbeiro</b><span>{item.barbeiroId ? ((barbeiroList.find(o => o.id === item.barbeiroId) || {}).nome ?? ("#" + item.barbeiroId)) : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Corte</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">barbeiroId</label>
      <input type="number" value={form["barbeiroId"] ?? ""} onChange={e => setForm({...form, barbeiroId: e.target.value})} />
      ...
      <label className="field-label">avaliacao</label>
      <input type="number" step="0.01" value={form["avaliacao"] ?? ""} onChange={e => setForm({...form, avaliacao: e.target.value})} />
      <label className="field-label">barbeiro</label>
      <select value={form["barbeiro"] ?? ""} onChange={e => setForm({...form, barbeiro: e.target.value})}>
        <option value="">Selecione...</option>
        {(barbeiroList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
...
function PainelProduto({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "marca", "categoria", "descricao", "preco", "estoque", "estoqueMinimo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "marca", "categoria", "descricao", "preco", "estoque", "estoqueMinimo"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/produto" + item.id + "/320/200"} alt="Produto" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">📦</div>
              <span className="item-id-tag">#{item.id}</span>
            ...
            <div className="item-field"><b>estoque</b><span>{item["estoque"] ?? "—"}</span></div>
            <div className="item-field"><b>estoqueMinimo</b><span>{item["estoqueMinimo"] ?? "—"}</span></div>
      ...
      <label className="field-label">estoqueMinimo</label>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Barbeiro");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Barbearia<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Barbeiro" ? " active" : "")} onClick={() => setAba("Barbeiro")}><span className="nav-ico">🧑‍🔧</span>Barbeiro</button>
        <button className={"nav-btn" + (aba === "Cliente" ? " active" : "")} onClick={() => setAba("Cliente")}><span className="nav-ico">👤</span>Cliente</button>
        <button className={"nav-btn" + (aba === "Corte" ? " active" : "")} onClick={() => setAba("Corte")}><span className="nav-ico">🗂️</span>Corte</button>
      ...
      <main className="main">
        ...
        {aba === "Barbeiro" && <PainelBarbeiro token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Cliente" && <PainelCliente token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Corte" && <PainelCorte token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
